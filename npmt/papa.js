...
app.get('/', function (req, res) {
    var url = 'http://www.adidas.com.cn';
    var items = [];
    superAgent
        .get(url)
        .set(headers)
        ...
        $('.productTile').each(function (index, item) {
            var $item = $(this);
            // 每一篇文章的url
            var itemURL = $item.find('.productLink').attr('href');
            var image = $item.find('.image > img').attr('data-img');
            var title = $item.find('.productLink').html();
            var price = $item.find('.cf p').html();
            items.push({
                index: index,
                image: image,
                itemURL: itemURL,
